...
async function uploadToPostImages(imagePath: string): Promise<string> {
  ...
  try {
    ...
    // Get the direct link
    const directLink = await page.$eval('input[id="code_direct"]', (el: HTMLInputElement) => el.value);
    
    // Get the gallery link (which is what we want to store in Airtable, as it's what our server handles)
    const galleryLink = await page.$eval('input[id="code_web"]', (el: HTMLInputElement) => el.value);
 ...
 */
async function processArticleRecord(record: any): Promise<void> {
